...
drupal_set_html_head('<link href="//fonts.googleapis.com/css?family=Raleway:400,900,800,700,600,500,300,200,100" rel="stylesheet" type="text/css" />');
...
function joycone_preprocess_page(&$vars) {
  ...
  if ((arg(0) === 'nodeorder' || arg(0) === 'taxonomy') && arg(1) == 'term' && is_numeric(arg(2))) {
    ...
    if (in_array($vid, array('1'))) {
      ...
      if ($tid == JOYCONE_PRODUCTS_TAXONOMY_RETAIL) $vars['head_title'] = 'Products | Joy Cone';
      if ($tid == JOYCONE_PRODUCTS_TAXONOMY_FS) $vars['head_title'] = 'Food Service Products | Joy Cone';
...
function joycone_taxonomy_term_page($tids, $result) {
  ...
  } else if (count($tids) === 1 && $tids[0] == JOYCONE_FAQ_TAXONOMY) {
    $result = db_query(db_rewrite_sql('SELECT t.tid, t.name FROM {term_hierarchy} th LEFT JOIN {term_data} t ON th.tid = t.tid WHERE th.parent = '.$tids[0].' ORDER BY t.weight ASC', 't', 'tid'));
    $terms = array();
    while ($term = db_fetch_object($result)) {
      $terms[] = $term;
    }
